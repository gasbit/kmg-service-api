...
สถานะ: Checklist ready — pending contract decisions  
...
- [ ] ยืนยัน exchange price mapping: `unitPrice = exchangeSalePrice`, `costPrice = exchangeCostPrice`
- [ ] ยืนยัน buy-full-tank mapping: `unitPrice = fullTankPrice`, `costPrice = exchangeCostPrice` ชั่วคราว และบันทึกข้อจำกัดว่า Product ยังไม่มี full-tank cost โดยเฉพาะ
- [ ] ยืนยัน borrow item pricing: `unitPrice = 0.00`, `costPrice = exchangeCostPrice`, `lineTotal = 0.00`
- [ ] ยืนยันว่าไม่รวม `depositAmount` ใน `totalAmount` และ Dashboard sales
...
- [ ] อัปเดต decisions ที่อนุมัติแล้วกลับเข้า `transaction-spec.md`

หากไม่อนุมัติ price/deposit mapping ห้าม implement total calculation โดยเดาเอง ให้อัปเดต `transaction-spec.md` ก่อน
