...
const TEST_DATABASE_NAME = "kmg_service_queues_test";
...
async function main(): Promise<void> {
  ...
  runNode(path.resolve("node_modules/vitest/vitest.mjs"), [
    "run",
    "src/modules/queue/queue.integration.test.ts"
  ], environment);
}

main().catch((error: unknown) => {
  console.error(error instanceof Error ? error.message : "Queue integration test setup failed");
  process.exit(1);
});
