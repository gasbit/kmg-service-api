...
const TEST_DATABASE_NAME = "kmg_service_transactions_test";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!existing[0]?.exists) {
      await admin.$executeRawUnsafe(`CREATE DATABASE "${testDatabase.name}"`);
    }
  } finally {
  ...
  runNode(path.resolve("node_modules/vitest/vitest.mjs"), [
    "run",
    "src/modules/transactions/transaction.integration.test.ts"
  ], environment);
}

main().catch((error: unknown) => {
  console.error(error instanceof Error ? error.message : "Transaction integration test setup failed");
  process.exit(1);
});
